...
tfidf_vectorizer = load_from_pickle(f'{filePath}/pickle_files/tfidf_vectorizer.pkl')
model = load_from_pickle(f'{filePath}/pickle_files/model_final.pkl')
recommendation_matrix = load_from_pickle(f'{filePath}/pickle_files/user_final_rating.pkl','rb')
products_data = load_from_pickle(f'{filePath}/pickle_files/products_data.pkl','rb')
...
def get_recommended_products(username):
    recommendations = recommendation_matrix.loc[user_input].sort_values(ascending=False)[0:20]
    ...
    print("*"*50)
    print(tabulate(top5_recommendations,headers="keys", tablefmt="grid"))
